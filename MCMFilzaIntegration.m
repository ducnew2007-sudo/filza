...
static BOOL MCMIsIOS26_1OrLater(void) {
    static BOOL result;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        result = ([[[UIDevice currentDevice] systemVersion] compare:@"26.1"
...
static void MCMInstallSystemRootLinks(NSFileManager *fm, NSString *root);
static void MCMInstallDirectFilesystemLinks(NSString *directory, NSString *containerRoot);
...
void MCMFilzaSetUnrestrictedFilesystem(BOOL enabled)
{
    ...
    if (enabled) {
        ...
        NSString *appBundles = [root stringByAppendingPathComponent:@"[MHA-C1] App Bundles"];

        for (NSString *d in @[root, apps, groups, extensions, vpnData, serviceData,
                              systemData, systemGroups, protectedData, appBundles]) {
            [fm createDirectoryAtPath:d withIntermediateDirectories:YES
                           attributes:@{NSFilePosixPermissions: @0700} error:nil];
        }

        MCMInstallDirectFilesystemLinks(apps, @"/private/var/mobile/Containers/Data/Application");
        MCMInstallDirectFilesystemLinks(groups, @"/private/var/mobile/Containers/Shared/AppGroup");
        MCMInstallDirectFilesystemLinks(extensions, @"/private/var/mobile/Containers/Data/PluginKitPlugin");
        MCMInstallDirectFilesystemLinks(vpnData, @"/private/var/mobile/Containers/Data/VPNPlugin");
        MCMInstallDirectFilesystemLinks(serviceData, @"/private/var/mobile/Containers/Data/InternalDaemon");
        MCMInstallDirectFilesystemLinks(systemData, @"/private/var/mobile/Containers/Data/System");
        MCMInstallDirectFilesystemLinks(systemGroups, @"/private/var/mobile/Containers/Shared/SystemGroup");
        MCMInstallDirectFilesystemLinks(protectedData, @"/private/var/mobile/Containers/Data/Protected");
        MCMInstallDirectFilesystemLinks(appBundles, @"/private/var/containers/Bundle/Application");
        MCMInstallSystemRootLinks(fm, root);
        NSLog(@"[MCMFilza] Unrestricted filesystem mode active, links populated");
    }
}
...
{
    MCMEnsureState();
    if (!gUnrestrictedFilesystem && ![MCMSignedCodeIdentifier() isEqualToString:kRequiredIdentifier]) {
    ...
    @synchronized (gLeases) {
        ...
        // iOS 26.1+ may resolve container paths through symlink chains that
        // O_NOFOLLOW rejects. Retry without it before giving up.
        if (descriptor < 0 && MCMIsIOS26_1OrLater()) {
            descriptor = open(lease.rootPath.fileSystemRepresentation,
                              O_RDONLY | O_DIRECTORY | O_CLOEXEC);
            if (descriptor >= 0)
                NSLog(@"[MCMFilza] open succeeded without O_NOFOLLOW id=%@", identifier);
        }
        if (descriptor < 0) {
            // On iOS 26.1+ the container path from containermanagerd is still
            // valid for browsing even when sandbox extension activation fails.
            // Keep the lease in browse-only mode so symlinks are created.
            if (MCMIsIOS26_1OrLater() && lease.rootPath.length > 0) {
                NSLog(@"[MCMFilza] browse-mode lease class=%llu id=%@ path=%@ (open errno=%d)",
                      containerClass, identifier, lease.rootPath, errno);
...
static void MCMInstallSystemRootLinks(NSFileManager *fm, NSString *root)
{
    NSArray<NSDictionary *> *systemLinks = @[
        @{@"Name": @"[FS] Root (/)", @"Target": @"/"},
        @{@"Name": @"[FS] Var (/var)", @"Target": @"/private/var"},
        @{@"Name": @"[FS] Mobile Home (/var/mobile)", @"Target": @"/private/var/mobile"},
        @{@"Name": @"[FS] Mobile Containers", @"Target": @"/private/var/mobile/Containers"},
        @{@"Name": @"[FS] System Containers", @"Target": @"/private/var/containers"},
        @{@"Name": @"[FS] App Bundles", @"Target": @"/private/var/containers/Bundle/Application"},
    ];
    for (NSDictionary *entry in systemLinks) {
        NSString *linkPath = [root stringByAppendingPathComponent:entry[@"Name"]];
        NSString *target = entry[@"Target"];
        struct stat st = {0};
        if (lstat(linkPath.fileSystemRepresentation, &st) == 0) {
            if (S_ISLNK(st.st_mode)) unlink(linkPath.fileSystemRepresentation);
            else continue;
        }
        symlink(target.fileSystemRepresentation, linkPath.fileSystemRepresentation);
    }
}
...
static NSString *MCMDirectContainerPath(uint64_t containerClass,
                                        NSString *identifier, BOOL group)
{
    if (!MCMBridgeAvailable() || !MCMSafeIdentifier(identifier)) return nil;
    NSString *detail = nil;
    MCMLease *lease = [MCMLease leaseForClass:containerClass identifier:identifier
        group:group part:0 flags:kMCMFlags error:&detail];
    if (!lease || !lease.rootPath.length) return nil;
    NSString *path = lease.rootPath;
    [lease invalidate];
    return path;
}
                                             ...
                                             BOOL logFailure)
{
    ...
    if (!target && MCMIsIOS26_1OrLater()) {
        target = MCMDirectContainerPath(containerClass, identifier, group);
        if (target)
            NSLog(@"[MCMFilza] fallback direct path class=%llu id=%@ path=%@",
                  containerClass, identifier, target);
    }
    if (!target) {
        if (logFailure)
...
{
    if (!gUnrestrictedFilesystem && !MCMIsIOS26_1OrLater()) return;
    NSFileManager *manager = NSFileManager.defaultManager;
...
void MCMFilzaStart(void)
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        BOOL hasMHA = [actual isEqualToString:kRequiredIdentifier];
        if (!hasMHA && !gUnrestrictedFilesystem) {
            NSLog(@"[MCMFilza] running with identifier %@ (bundle=%@)",
                  actual, NSBundle.mainBundle.bundleIdentifier);
        }
        if (!MCMBridgeAvailable() && !gUnrestrictedFilesystem) {
            NSLog(@"[MCMFilza] ContainerManager symbols unavailable");
        }
        NSFileManager *fm = NSFileManager.defaultManager;
        NSString *root = MCMFilzaVirtualRoot();
        ...
        NSString *experimental = [root
            stringByAppendingPathComponent:kMCMExperimentalDirectoryName];
        NSString *appBundles = [root
            stringByAppendingPathComponent:@"[MHA-C1] App Bundles"];
        ...
        MCMInstallDirectFilesystemLinks(appBundles,
