...
static void hook_fileSystemViewWillAppear(id self, SEL _cmd, BOOL animated) {
    ...
    if (redirected && ![redirected isEqual:currentPath]) {
        ...
        NSLog(@"[DeviceStorage] repaired legacy browser path from %@ to %@",
            currentPath, redirected);
    }
    ((void(*)(id, SEL, BOOL))orig_fileSystemViewWillAppear)(self, _cmd, animated);

    if (firstAppearance && (!currentPath || currentPath.length == 0)) {
        ((void(*)(id, SEL, id))objc_msgSend)(self,
...
static BOOL repairActiveBrowserPath(void) {
    ...
    if (!currentPath || currentPath.length == 0 || [currentPath containsString:@"/Documents/MCM Containers"]) {
        NSString *target = redirectedLegacyBrowserPath(currentPath) ?: root;
        ((void(*)(id, SEL, id))objc_msgSend)(controller,
            setCurrentPathSelector, target);
        controller.navigationItem.title = @"Device Storage";
        NSLog(@"[DeviceStorage] initialized active browser path from %@ to %@ class=%@",
            currentPath, target, NSStringFromClass(controller.class));
        SEL loadSelector = NSSelectorFromString(@"doLoadingPage");
        if ([controller respondsToSelector:loadSelector]) {
            ((void(*)(id, SEL))objc_msgSend)(controller, loadSelector);
            NSLog(@"[DeviceStorage] reloaded active browser path %@ class=%@",
                  target, NSStringFromClass(controller.class));
        }
    }
    return YES;
}
...
static void attemptSandboxEscape(void) {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        NSLog(@"[Tweak] Checking sandbox status...");
        int fd_w = open("/var/mobile/.sbx_test", O_WRONLY | O_CREAT | O_TRUNC, 0644);
        if (fd_w >= 0) {
            close(fd_w);
            unlink("/var/mobile/.sbx_test");
            NSLog(@"[Tweak] Already unsandboxed!");
            MCMFilzaSetUnrestrictedFilesystem(YES);
            return;
        }

        NSLog(@"[Tweak] Attempting kernel exploit (opa334) & sandbox escape...");
        @try {
            ...
            if (ret == 0) {
                ...
                if (self_proc) {
                    ...
                    if (sbx_ret == 0) {
                        sandbox_elevate_to_root(self_proc);
                        MCMFilzaSetUnrestrictedFilesystem(YES);
                        NSLog(@"[Tweak] Sandbox escape succeeded! Full /var & container access enabled.");
                    } else {
                        NSLog(@"[Tweak] sandbox_escape returned %d", sbx_ret);
                    }
                } else {
                    NSLog(@"[Tweak] proc_self() returned 0");
                }
            } else {
                NSLog(@"[Tweak] kexploit_opa334 returned %d", ret);
            }
        } @catch (NSException *e) {
            NSLog(@"[Tweak] Exception in kernel exploit / sandbox escape: %@", e);
        }
    });
}
